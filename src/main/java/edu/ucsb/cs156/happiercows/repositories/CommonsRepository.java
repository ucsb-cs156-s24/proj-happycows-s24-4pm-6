...
public interface CommonsRepository extends CrudRepository<Commons, Long> {
    @Query("SELECT sum(uc.numOfCows) from user_commons uc where uc.id.commons.id = :commonsId")
    Optional<Integer> getNumCows(Long commonsId);

    @Query("SELECT COUNT(*) FROM user_commons uc WHERE uc.id.commons.id = :commonsId")
